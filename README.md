...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create remix-bafd
  fly apps create remix-bafd-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app remix-bafd
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app remix-bafd-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'remix-bafd-staging' does not match app name in config file 'remix-bafd'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name remix-bafd-db
  fly postgres attach --app remix-bafd remix-bafd-db

  fly postgres create --name remix-bafd-staging-db
  fly postgres attach --app remix-bafd-staging remix-bafd-staging-db
  ```
